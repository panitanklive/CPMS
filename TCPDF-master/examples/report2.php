...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        // Title
        $this->Ln(5);
        $this->Cell(385, 5, 'รายงานการใช้เชื้อเพลิง  ตั้งแต่วันที่ : '.$trip_start.' ถึง '.$trip_end.'                                               หน้า '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'M', 'M');
		
        $this->Ln();
		$this->SetFont('thsarabun', '', 14, '', true);
		$this->Cell(290, 5, 'ค้นหาจากทะเบียนรถยนต์ : '.$carpool_id, 0, false, 'C', 0, '', 0, false, 'M', 'M');
		 $this->Ln();
...
$pdf->SetTitle('รายงานการใช้เชื้อเพลิง/การไฟฟ้าส่วนภูมิภาค เขต 1 (ภาคกลาง)');
...
function body(){
	...
	<thead>
		...
		<td width="14%" bgcolor="#F2F2F2" align="center"><b>สถานที่</b></td>
		<td width="11%" bgcolor="#F2F2F2" align="center"><b>ระยะทางที่ใช่้รถ</b></td>
		<td width="11%"  bgcolor="#F2F2F2" align="center"><b>จำนวนน้ำมันที่ใช้</b></td>
		<td width="11%"  bgcolor="#F2F2F2" align="center"><b>ราคาสุทธิ</b></td>
...
function feed(){
	...
	if(!empty($trsg_id)){
		...
		$km = intval($row["trip_mile_end"]) - intval($row["trip_mile"]);
		...
		$oil_liter = 0;
		$oil_net = 0;
		$sql3="SELECT oil_liter,oil_net from tb_oil where trip_id ='".$row["trip_id"]."'";
		$result3 = mysqli_query($conn,$sql3);
		if(mysqli_num_rows($result3)>0){
			while($rs = $result3->fetch_array(MYSQLI_ASSOC)){
				$oil_liter = $oil_liter + intval($rs["oil_liter"]);
				$oil_net = $oil_net + intval($rs["oil_net"]);
		}}	
		...
		<td width="14%" align="center">'.$row["trsg_name"].'</td>
		<td width="11%" align="center">'.$km.' กม.</td>
		<td width="11%" align="center">'.$oil_liter.' ลิตร</td>
		<td width="11%" align="center">'.$oil_net.' บาท.</td>
		</tr>'; 				
}
