...
$i=0;
	...
	include("../../myconn/myconn.php");
        $sql="select * FROM tb_passenger WHERE trip_id = '$trip_id' and passenger_status='จอง'";
...
$output .='
 <tr>
	...
	<td align="center">'.$row["employee_id"].'</td>
  	<td align="left">&nbsp;&nbsp;'.$row["employee_detail"].'</td>
	<td align="center">'.$row["passenger_tel"].'</td>
	<td align="center">'.$row["passenger_tel_table"].'</td>
	<td align="center">'.$row["employee_dep"].'</td>
	<td align="center">'.$row["passenger_status"].'</td>
  </tr>';
}
$var = 1-$i;
for ($i1=0; $i1 < $var ; $i1++) { 
...
$i=0;
	$output ='';
	include("../../myconn/myconn.php");
        $sql="select * FROM tb_oil  WHERE trip_id = '$trip_id'";
		$result = mysqli_query($conn,$sql);
		while($row = $result->fetch_assoc()){
				 	$i++;
$output .='
...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        // Title
        $this->Ln(5);
        $this->Cell(183, 5, 'แบบฟอร์มยืม-คืนรถยนต์ ', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Ln();
...
$pdf->SetTitle('แบบฟอร์มขอยืมใช้รถยนต์/การไฟฟ้าส่วนภูมิภาค เขต 1 (ภาคกลาง) ');
...
function ochem_id(){
	...
	$html1 =''; 
	include("../../myconn/myconn.php");
        $sql="select * FROM tb_trip NATURAL JOIN tb_car NATURAL JOIN tb_trsg WHERE trip_id = '$trip_id'";
		$result = mysqli_query($conn,$sql);
		while($row = $result->fetch_assoc()){
...
<head>
	...
	<b align ="right">เลขที่ใบจองรถ :  '.$trip_id.'</b>
 ...
 $html1 .=  '
			<h4><b>ข้อมูลการเติมน้ำมัน</b></h4><br>
		<div>	
		<table border="1" width="100%">
    	<tr>
 ...
 $html1.='</table>
		...
		<h4><b>แผนที่แสดงตำแหน่งรถยนต์ที่จอด</b></h4><br>
		
		<table border="0" width="100%">&nbsp;';
		
		if($carpool_park == 'อาคาร 1'){
			$html1 .='<img src="images/park/map0.jpg" width="700px" align ="center"></img>';
		}
		else if($carpool_park == 'อาคาร 2'){
			$html1 .='<img src="images/park/map1.jpg" width="700px" align ="center"></img>';
		}
		else if($carpool_park == 'อาคาร 3 Scarda'){
			$html1 .='<img src="images/park/map2-1.jpg" width="700px" align ="center"></img>
';
		}
		else if($carpool_park == 'อาคาร 5'){
			$html1 .='<img src="images/park/map2.jpg" width="700px" align ="center"></img>';
		}
		else if($carpool_park == 'อาคาร 6'){
			$html1 .='<img src="images/park/map4.jpg" width="700px" align ="center"></img>';
		}
		else if($carpool_park == 'ลานกว้าง'){
			$html1 .='<img src="images/park/map5.jpg" width="700px" align ="center"></img>';
		}
		else if($carpool_park == 'ลานอเนกประสงค์'){
			$html1 .='<img src="images/park/map6.jpg" width="570px" align ="center"></img>';
		}
		else if($carpool_park == 'โรงอาหาร'){
			$html1 .='<img src="images/park/map7.jpg" width="700px" align ="center"></img>';
		}
		else if($carpool_park == 'สถานนีไฟฟ้า'){
			$html1 .='<img src="images/park/map8.jpg" width="700px" align ="center"></img>';
		}else{
			$html1 .='<img src="images/park/map.jpg" width="700px" align ="center"></img>';
		}
$html1.='
		</table>
	';
