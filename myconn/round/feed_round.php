...
$date = date("Y-m-d");
$time = date("H:i");
$setdate = date("Y-m-d").' '.date("H:i");

$query = "SELECT * from tb_car natural join tb_trsg natural join tb_trip where trip_status = 0 order by trip_start asc";
...
if(mysqli_num_rows($result)>0){
	while($rs = $result->fetch_array(MYSQLI_ASSOC)){
		if ($outpb != "") {$outpb .= ",";}
			...
			$fulldate = $rs["trip_start"].' '.$rs["trip_time"];
			if($setdate >= $fulldate){
				$outpb .= '"limit":"true",';
			}else{
				$outpb .= '"limit":"false",';
